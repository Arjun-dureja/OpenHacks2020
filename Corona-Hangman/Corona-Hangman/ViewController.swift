...
class ViewController: UIViewController {
    ...
    @objc func loadGame() {
        ...
        if let hintsFileURL = Bundle.main.url(forResource: "hints", withExtension: ".txt") {
            if let hintsContents = try? String(contentsOf: hintsFileURL) {
                hints = hintsContents.components(separatedBy: "\n")
            }
        }
    ...
    @objc func hintPressed() {
        let ac = UIAlertController(title: "Hint", message: hints[currentIndex!], preferredStyle: .alert)
